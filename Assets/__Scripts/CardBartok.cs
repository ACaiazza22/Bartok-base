...
public class CardBartok : Card {
	...
	public CBState state = CBState.drawpile; // Fields to store info the card will use to move and rotate 
	...
	public GameObject reportFinishTo = null; // MoveTo tells the card to interpolate to a new position and rotation 
	public void MoveTo( Vector3 ePos, Quaternion eRot) { // Make new interpolation lists for the card. // Position and Rotation will each have only two points. 
		...
		timeDuration = MOVE_DURATION; // Setting state to either toHand or toTarget will be handled by the 
		// calling method 
		state = CBState.to; 
	} 
	...
	void Update() { 
		switch (state) { // All the to___ states are ones where the card is interpolating 
		case CBState.toHand: 
		...
		case CBState.to:
			...
			if (u < 0) { // If u < 0, then we shouldn't move yet. 
				...
				if (state == CBState.to)
					state = CBState.idle; // Move to the final position 
				transform.localPosition = bezierPts [bezierPts.Count - 1]; 
				transform.rotation = bezierRots [bezierPts.Count - 1]; // Reset timeStart to 0 so it gets
				timeStart = 0; 
				if (reportFinishTo != null) { // If there's a callback GameObject 
					...
					reportFinishTo.SendMessage (" CBCallback", this); // After calling SendMessage(), reportFinishTo must be set // to null so that it the card doesn't continue to report 
