...
public class BartokLayout : MonoBehaviour {
	...
	public SlotDef target; // This function is called to read in the LayoutXML.xml file 
	...
	public void ReadLayout( string xmlText) { 
		...
		multiplier.y = float.Parse (xml ["multiplier"] [0].att ("y")); // Read in the slots
		SlotDef tSD; // slotsX is used as a shortcut to all the < slot > s 
		...
		for (int i = 0; i < slotsX.Count; i ++) { 
			...
			tSD.layerName = tSD.layerID.ToString (); // The layers are used to make sure that the correct cards are 
			...
			switch (tSD.type) {
			case "slot": // ignore slots that are just of the "slot" type 
				break; 
			case "drawpile": // The drawPile xstagger is read but not actually used in Bartok 
			...
			case "discardpile":
				discardPile = tSD; 
				break; 
			case "target": // The target card has a different layer from discardPile 
				target = tSD; 
				break; 
			case "hand": // Information for each player's hand 
